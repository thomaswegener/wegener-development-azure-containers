...
import { sampleAboutProfiles, sampleCalendarEvents } from './sampleData';
...
export class NotionContentService {
    ...
    async getCalendarResponse() {
        ...
        const events = results
            .map((page) => this.isFullPage(page) ? this.toCalendarEvent(page) : null)
    ...
    toAboutProfile(page) {
        const nameProperty = page.properties[this.aboutNameProperty] ?? page.properties.Name;
        if (!nameProperty ||
...
const pickMultiSelect = (property) => {
    if (!property)
        return [];
...
const pickAudienceList = (property) => {
    if (!property)
        return [];
    if ('multi_select' in property) {
        return property.multi_select.map((item) => item.name);
...
const pickPeopleOrText = (property) => {
    if (!property)
        return [];
